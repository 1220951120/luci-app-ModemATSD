...
allowed_path() {
    case "$1" in
        etc/config/modem-AK68|etc/modem-traffic-AK68.state|usr/bin/smstrun-AK68.conf|usr/bin/smstrun-title-AK68.conf|usr/bin/ledflagc.conf) return 0;;
        *) return 1;;
    esac
}
...
restore_backup() {
    ...
    rm -f /etc/modem-traffic-AK68.state /usr/bin/smstrun-AK68.conf /usr/bin/smstrun-title-AK68.conf /usr/bin/ledflagc.conf
    while IFS= read -r path; do
        path="${path#./}"
    ...
    chmod 600 /etc/modem-traffic-AK68.state /usr/bin/smstrun-AK68.conf /usr/bin/smstrun-title-AK68.conf 2>/dev/null || true
